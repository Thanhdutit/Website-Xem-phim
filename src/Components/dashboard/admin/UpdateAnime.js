...
function AddMovie() {
    ...
    useEffect(() => {
        ...
        const getCategories = async () => {
            let categories = await axios.get('http://localhost:1001/categoryAnimes')
            setListCategories(categories.data.data)
            setCategory(categories.data.data[0].id)
        }
        const getAuthor = async () => {
            let author = await axios.get('http://localhost:1001/authors/animes')
            setListAuthor(author.data)
            setAuthor(author.data[0].id)
        }
        const getFilmByID = async () => {
            let result = await axios.get(`http://localhost:1001/animes/${id}`)
    ...
    const HandleClickSubmit = async () => {
        let isSuccess = window.confirm('BẠN CÓ CHẮC CHẮN MUỐN SỬA ANIME')
        if (isSuccess) {
    ...
    async function HandleAPIUpdate() {
        ...
        let data = await axios.put(`http://localhost:1001/animes/${filmID}`,
            {
                name: name,
                author_id: author,
    ...
    const HandleDeteleCharacterBtn = async (data) => {
        ...
        if (isDelete) {
            let data1 = await axios.delete(`http://localhost:1001/animes/character/${data.id}`)
    ...
    return (
        <SideBarHome>
            ...
            <div className='flex flex-col gap-6 add-film-home' >
                <h2 className='text-xl font-bold'>Sửa anime</h2>
