...
import '../style.css'
let url = 'http://localhost:1001/animeitems/'
function ListMovieEpisode() {
    ...
    const OnDeletedFunction = async (data) => {
        ...
        if (isDelete) {
            let data1 = await axios.delete(`http://localhost:1001/animeitems/${data.id}`)
    ...
    return (
        <SideBarHome>
            <EpisodeModal setModalOpen={setModalOpen} modalOpen={modalOpen} data={dataEpisode} anime={true} />
