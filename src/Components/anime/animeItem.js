...
function AnimeItem() {
    ...
    const HandleClickLike = async () => {
        ...
        stateBtn? await axios.delete(`http://localhost:1001/accounts/favoriteAnimes/${userID}/delete/${id}`)
        : await axios.post(`http://localhost:1001/accounts/favoriteAnimes/${userID}`, {
            anime_id: id
        })
    ...
    const getAnime = async (anime) => {
        const response = await fetch(`http://localhost:1001/animes/${anime}`)
        const data = await response.json()
        setAnime(data)
        const cate = await axios('http://localhost:1001/categoryAnimes/'+data.categories_id)
        setCate(cate.data.name)
        const au = await axios.get('http://localhost:1001/authors/animes/'+data.author_id)
        setAuthor(au.data.data.name)
    }

    const getCharacters = async (id) => {
        let characters = await axios.get(`http://localhost:1001/animes/${id}/character`)
        setCharacters(characters.data)
    }
    useEffect(() => {
        getAnime(id)
        setHref("/anime/" + id + "/details/1")
        getCharacters(id)
        ...
        async function getData() {
            let result = await axios(`http://localhost:1001/accounts/favoriteAnimes/${userID}`)
                ...
                <div className="details">
                    <div className="detail">
                        ...
                        <div className="anime-details">
                            ...
                            <p><span>Tác giả:</span><span>{author}</span></p>
