...
function MainModals({modalOpen,setModalOpen,children,setImage,setFile,setEpisode}) {
    const canceButtonRef = useRef();
  return (
    <>
    <Transition show={modalOpen} as={Fragment} appear>
        <Dialog 
        as ="div" className="fixed inset-0 z-30 overflow-y-auto text-center  "
         initialFocus={canceButtonRef} onClose={() => {
            
            if(setEpisode) setEpisode(null)
